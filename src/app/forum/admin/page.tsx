...
import type { AdminStats } from '@/lib/forum/types';
...
const RATE_LIMIT_FIELDS: { key: string; label: string; unit: string }[] = [
  ...
  { key: 'verify_email_resend_cooldown_seconds', label: '验证邮件重发间隔（同一邮箱）', unit: '秒' },
  { key: 'forgot_password_cooldown_seconds', label: '找回密码冷却（同一 IP）', unit: '秒' },
  { key: 'change_email_cooldown_seconds', label: '更换邮箱冷却（同一 IP）', unit: '秒' },
              ...
              <div key={uid} className="py-2 border-b last:border-0">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-3 min-w-0">
                    ...
                    {u.verified ? <span className="text-xs text-green-600 shrink-0">已验证</span> : <span className="text-xs text-amber-600 shrink-0">未验证</span>}
                  </div>
