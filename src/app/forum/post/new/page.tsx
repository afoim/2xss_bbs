...
import { createPost, getCategories, uploadFile } from '@/lib/forum/api/client';
        ...
        <div className="flex gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-40">
              <DropdownMenuItem onClick={() => setCategoryId('')}>选择分类</DropdownMenuItem>
              {categories.map((c) => (
                <DropdownMenuItem key={c.id} onClick={() => setCategoryId(c.id)}>{c.name}</DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
