...
import { useForumAuth } from '@/lib/forum/stores/auth';
import {
...
} from '@/lib/forum/api/client';
import { parseCommentSort, type CommentSort } from '@/lib/forum/api/map-comment';
import type { ForumPostDetail, ForumComment, ForumCategory } from '@/lib/forum/types';

/** 评论可能带服务端渲染好的 html（SSR 首屏），也可能没有（客户端拉取的） */
type ForumCommentWithHtml = ForumComment & { html?: string; replies?: ForumCommentWithHtml[] };

import { remoteThumb } from '@/lib/cover-thumb';
import { toast } from 'sonner';
import { ImageLightbox } from '@/components/image-lightbox';
import { BackToList } from '@/components/back-to-list';
import { MermaidContent } from '@/components/mermaid-renderer';
import { Skeleton } from '@/components/ui/skeleton';
// 编辑器走懒加载壳：直接 import '@/components/markdown-editor' 会把 74KB 编辑器
...
import { loadRenderer, useRenderer } from '@/lib/forum/markdown-client';
import { applySeo, makeExcerpt } from '@/lib/seo/apply-seo';
import { SITE_URL, canonicalPath } from '@/lib/seo/route-meta';
...
export function PostContent({
  ...
  const handlePostLike = useCallback(async () => {
    ...
    try {
      ...
      setPostLikeCount((prev) => r.liked ? prev + 1 : Math.max(0, prev - 1));
    } catch (e: unknown) {
      toast.error('点赞失败', { description: e instanceof Error ? e.message : '请稍后再试' });
    }
  }, [post]);
